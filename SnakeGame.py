...
class environment(py_environment.PyEnvironment):
    ...
    _action_spec = array_spec.BoundedArraySpec(shape=(),dtype=np.int32,minimum=0,maximum=3,name="action")
    _observation_spec = array_spec.BoundedArraySpec(shape=(230,),dtype=np.float32,minimum=0.0,maximum=1.0,name="observation")
    ...
    def _step(self,action):
        ...
        if self._episode_ended:
            ...
            return ts.transition(np.array(self._state, dtype= np.float32),reward,discount = 0.95)
    def _reset(self):
        self._state = self.reset_env()
    ...
    def combineData(self,spaceFruit):
        ...
        finalList.append(((self.snakeHead[0])/16))
        finalList.append(((self.snakeHead[1])/16))
        for i in range(1,16):
            for j in range(1,16):
                if self.inSnake([i,j]):
                    finalList.append(1.0)
                else:
